 ...
 * Content Posting API. Routes stay thin and never talk to TikTok directly.
 *
 * Docs:
 ...
 *
 * Every external response is validated with zod so a malformed/changed payload
 * fails loudly at the boundary instead of leaking `any` through the app.
 */
...
/** Source of the media for an upload. */
...
async function tiktokFetch<S extends z.ZodTypeAny>(
  ...
  if (!parsed.success) {
    throw new TikTokApiError(
      `Unexpected TikTok payload shape: ${parsed.error.issues.map(i => i.path.join(".")).join(", ")}`,
      502,
    );
  }
  return parsed.data;
...
/** Some Content Posting endpoints return error.code === 'ok' on success. */
...
/* ------------------------------------------------------------------ *
 *  Display API (Login Kit scopes: user.info.basic, video.list)
 *  These work with the currently-approved app.
 * ------------------------------------------------------------------ */
...
/** Fields requestable from /user/info/ with the user.info.basic scope. */
...
/**
 * Required before Direct Post: tells us which privacy levels the creator may
 * use and whether comment/duet/stitch are allowed for their account.
 */
...
/**
 * Initialize a Direct Post (publishes to the creator's profile).
 * Returns the publish_id (and an upload_url when source is FILE_UPLOAD).
 */
...
/**
 * Upload to the creator's TikTok inbox (drafts) so they can finish editing and
 * publish from the app. Uses the upload/init endpoint via the same init schema
 * but with no privacy/post settings (drafts).
 */
...
/**
 * Upload binary video bytes to the upload_url returned by a FILE_UPLOAD init.
 * TikTok expects a PUT with a Content-Range header per chunk. Here we send the
 * whole file as a single chunk (chunk_size === video_size).
 */
export async function uploadVideoBytes(
  ...
  try {
    ...
    throw new TikTokApiError(
      `Network error uploading bytes: ${(err as Error).message}`,
      502,
    );
  }
  if (!res.ok) {
    throw new TikTokApiError(`Byte upload failed (HTTP ${res.status})`, 502);
  }
}
