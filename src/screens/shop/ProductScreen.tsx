...
import { useDominantColor, colorName } from '@/hooks/useDominantColor';
...
function VariantSwatch({ image }: { image: string | null }) {
  const { hex, name, loading } = useDominantColor(image);
  return (
    <View style={[styles.variantSwatchWrap, { backgroundColor: hex || tokens.colors.surface }]}>
      {image ? <Image source={{ uri: image }} style={styles.variantSwatch} /> : null}
      {!loading && hex ? <View style={[styles.colorDot, { backgroundColor: hex }]} /> : null}
      {!loading && hex ? <Text style={styles.colorName}>{name}</Text> : null}
    </View>
  );
}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.variantRow}>{product.variants.map((v) => (
            <TouchableOpacity key={v.id} style={[styles.variantChip, variantId === v.id && styles.variantChipActive]} onPress={() => setVariantId(v.id)}>
              {v.image ? <VariantSwatch image={v.image} /> : null}
...
const styles = StyleSheet.create({
  ...
  variantSwatchWrap: { alignItems: 'center', gap: 2, minWidth: 44 },
  variantSwatch: { width: 44, height: 44, borderRadius: tokens.radius.xs, backgroundColor: tokens.colors.surface },
  colorDot: { width: 10, height: 10, borderRadius: 5, borderWidth: 1, borderColor: 'rgba(255,255,255,.35)' },
  colorName: { color: tokens.colors.text.tertiary, fontSize: 9, maxWidth: 44, textAlign: 'center' },
  variantChipBody: { gap: 2 }, variantText: { color: tokens.colors.text.secondary, fontSize: tokens.typography.body.fontSize }, variantTextActive: { color: tokens.colors.brand.primary, fontWeight: '700' }, variantStock: { color: tokens.colors.text.tertiary, fontSize: tokens.typography.caption.fontSize }, qtyRow: { flexDirection: 'row', alignItems: 'center', gap: tokens.spacing.md }, qtyBtn: { width: 36, height: 36, borderRadius: tokens.radius.sm, backgroundColor: tokens.colors.elevated, alignItems: 'center', justifyContent: 'center' }, qtyBtnDisabled: { opacity: .4 }, qtyBtnText: { color: tokens.colors.white, fontSize: 20 }, qtyValue: { color: tokens.colors.white, minWidth: 32, textAlign: 'center', fontWeight: '800' }, stockText: { color: tokens.colors.semantic.success, fontSize: tokens.typography.caption.fontSize, fontWeight: '700' }, stockTextOut: { color: tokens.colors.semantic.error, fontWeight: '700' }, description: { color: tokens.colors.text.secondary, fontSize: tokens.typography.body.fontSize, lineHeight: 20 },
